...
pub fn nonce_queried_event(env: &Env, account: &Address, nonce: u64) {
    env.events().publish((Symbol::new(env, "nonce_queried"),), (account, nonce));
}

/// Emit fee deducted event.
pub fn fee_deducted_event(env: &Env, from: &Address, fee_amount: i128) {
    env.events().publish((Symbol::new(env, "fee_deducted"),), (from, fee_amount));
}

/// Emit history appended event.
pub fn history_appended_event(env: &Env, from: &Address, from_clone: &Address, to: &Address, amount: i128) {
    env.events().publish((Symbol::new(env, "history_appended"),), (from, from_clone, to, amount));
}
...
pub fn fee_updated_event(env: &Env, old_bps: i128, new_bps: i128) {
    env.events().publish((Symbol::new(env, "fee_updated"),), (old_bps, new_bps));
}
